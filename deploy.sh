...
#   2. No data loss - it only ever rebuilds/restarts the `web` service.
#      `db` and `caddy` (and therefore pgdata / media_volume) are never
#      stopped, recreated, or passed to any `-v`-flagged command.
set -euo pipefail
...
# the `web` service's `env_file:` entry loads the same file into the
# container's actual runtime environment. No manual `source`/export step -
...
# --- 2. Rebuild + restart ONLY the app container ------------------------
# Deliberately `build web` / `up -d --no-deps web`, never a bare
# `docker compose up -d --build` (which would recreate db/caddy too) and
# never anything with `-v` (which would delete pgdata / media_volume).
# Recreating the `web` container re-runs its CMD from scratch, so
# collectstatic -> migrate -> setup_page_lock_schedule -> gunicorn all
# execute fresh - no separate static-refresh step needed here.

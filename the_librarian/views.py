...
def archive_list(request):
    """
    List all PDF files in the ARCHIVE_DIR.
    """
    ...
    return render(request, "the_librarian/archive_list.html", {"pdfs": pdfs})

@require_GET
def archive_viewer(request, filename):
    """
    View an uningested or raw PDF from the archive directory using ViewerJS.
    """
    from django.urls import reverse
...
def archive_download(request, filename):
    """
    Serve a PDF file directly from the archive directory by filename.
    """
    from django.http import FileResponse
    import os
